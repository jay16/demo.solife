var app = angular.module('solife', []);

app.controller('NxscaeCtrl', function($scope, $http) {
    var listType = "all";
    ...
    $http.get("/demo/nxscae/list?list="+listType).success(function(result) {
     ...
     $http.get("/demo/nxscae/focus").success(function(result) {
        $(".fullname").each(function(){
            var focus = result.array;
            var index = $.inArray($(this).html(), focus);
           if(focus.length > 0 && index >= 0) {
    ...
    $scope.trendChart = function(code) {
        $http.get("/demo/nxscae/" + code + "/data").success(function(result) {
