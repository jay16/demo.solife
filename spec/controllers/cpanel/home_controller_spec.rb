...
describe "Cpanel::HomeController" do
  ...
  it "should redirec to login page when without lgoin" do
    ...
    follow_redirect!
    expect(last_request.url).to eq(redirect_url("/users/login"))
  end
  ...
  it "should login successfully with incorrect information" do
  ...
  it "should login successfully with correct information" do
    if false and user = User.first
      post "/users/login",  { user: { email: user.email, password: user.password }}
      ...
      pending "FactoryGirl to mock<User>"
    end
  end
