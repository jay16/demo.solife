    ...
    module ReplyRobot
      module UtilsMethods
        def execute_callback(message, text)
          ...
          remark = "注: 执行%d次回调函数." % callbacks.count unless callbacks.empty?
          remark || ''
        end
      end
      # command parser
      class Command
        ...
        def handler
          if @raw_cmd =~ /^\?/ || @raw_cmd.force_encoding('UTF-8').start_with?("？")
            help
      ...
      module ReplyHelpers
        def reply_robot(message)
          ...
          Sinatra::SOLife::ReplyRobot::Parser.exec(message, options)
        end
      end
